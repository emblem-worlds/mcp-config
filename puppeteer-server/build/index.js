    ...
    setupToolHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            if (!['launch', 'close'].includes(request.params.name) && !state.browser) {
                throw new McpError(ErrorCode.InvalidRequest, 'Browser not launched. Call launch first.');
            }
            try {
                ...
                throw new McpError(ErrorCode.InternalError, `Puppeteer error: ${error instanceof Error ? error.message : String(error)}`);
            }
        });
    }
    async run() {
