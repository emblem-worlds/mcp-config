    ...
    setupHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            switch (request.params.name) {
                case 'create_node': {
                    const { labels, properties = {} } = request.params.arguments;
                    const node = {
                        id: this.generateId(),
                        labels,
                        properties,
                ...
                case 'create_relationship': {
                    const { startNodeId, endNodeId, type, properties = {} } = request.params.arguments;
                    if (!this.nodes.has(startNodeId)) {
                        throw new McpError(ErrorCode.InvalidRequest, `Start node not found: ${startNodeId}`);
                    }
                    if (!this.nodes.has(endNodeId)) {
                        throw new McpError(ErrorCode.InvalidRequest, `End node not found: ${endNodeId}`);
                    }
                    const relationship = {
                        id: this.generateId(),
                        type,
                ...
                case 'query_graph': {
                    ...
                    for (const node of this.nodes.values()) {
                        if (query.match?.nodeLabels) {
                            const hasAllLabels = query.match.nodeLabels.every(label => node.labels.includes(label));
                            if (!hasAllLabels)
                                continue;
                        }
                        if (query.match?.properties) {
                            const matchesProperties = Object.entries(query.match.properties).every(([key, value]) => node.properties[key] === value);
                        ...
                        if (query.match?.relationships) {
                            for (const relationship of this.relationships.values()) {
                                if (relationship.startNodeId === node.id ||
                                    ...
                                    const relTypeMatch = !query.match.relationships.some(r => r.type && r.type !== relationship.type);
                                    if (relTypeMatch) {
                ...
                case 'get_node': {
                    ...
                    if (!node) {
                        throw new McpError(ErrorCode.InvalidRequest, `Node not found: ${id}`);
                    }
                    return {
                        content: [
                            {
