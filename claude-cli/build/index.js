...
async function checkRateLimit(tokens) {
    try {
...
const createThrottledMessage = async (messages, isSimpleQuery = false) => {
...
program
    ...
    try {
        ...
        const response = await createThrottledMessage([{ role: 'user', content: prompt }], true);
